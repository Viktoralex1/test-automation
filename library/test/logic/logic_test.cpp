 ...
 */
template <std::uint16_t EepromSize = 1024U> struct Mock final {
  ...
  logic::Interface &createLogic() {
    logicImpl = std::make_unique<logic::Stub>(
        led, toggleButton, tempButton, debounceTimer, toggleTimer, tempTimer,
   ...
   * @brief Run system.
   *
   * @param[in] testDuration_ms Duration to run the system in ms (default = 10
   * ms).
   */
...
private:
  ...
  // -----------------------------------------------------------------------------
  static void stopLogicThread(const std::size_t timeout_ms,
                              bool &stop) noexcept {
...
TEST(Logic, DebounceHandling) {
  ...
  // Expect button interrupts to be disabled and the debounce timer to be
  // enabled. Expect the toggle timer to be enabled due to the toggle button
  // being pressed.
  {
  ...
  // Case 2 - Call the button event handler before the debounce timer has timed
  // out. Expect this call to be ignored, i.e, expect the toggle timer to still
  // be enabled.
  {
    mock.toggleButton.write(true);
    logic.handleButtonEvent();
  ...
  // Case 3 - Simulate debounce timer timeout, expect toggle button interrupt to
  // be re-enabled. Expect the debounce timer to be disabled.
  {
    EXPECT_TRUE(mock.debounceTimer.isEnabled());
  ...
  // Case 4 - Press the toggle button again after the debounce timer has timed
  // out, simulate button event. Expect button interrupts to be disabled and the
  // debounce timer to be enabled. Expect the toggle timer to be disabled due to
  // the toggle button being pressed.
  {
    mock.toggleButton.write(true);
    logic.handleButtonEvent();
    mock.toggleButton.write(false);
...
TEST(Logic, ToggleHandling) {
  ...
  // Expect the toggle timer to not be enabled, since the wrong button was
  // pressed.
  {
    // Press the button
    mock.tempButton.write(true);

    // Simulate button interrupt (we have to call it manually, since we don't
    // have real interrupts)
    logic.handleButtonEvent();

    // Realese the button
    mock.tempButton.write(false);

    // We pressed the wrong button, hence the toggle timer should still be
    // disabled.
    EXPECT_FALSE(mock.toggleTimer.isEnabled());

    // Simulate that the debounce timer times out, i.e. the buttons interrupts
    // "work" again.
    mock.debounceTimer.setTimedOut(true);
    logic.handleDebounceTimerTimeout();
  }
  ...
  // Case 4 - Simulate that the toggle timer elapses again, expect the LED to be
  // disabled.
  {
    mock.toggleTimer.setTimedOut(true);
    logic.handleToggleTimerTimeout();
  ...
  // Case 5 - Simulate that the toggle timer elapses once more, expect the LED
  // to be enabled.
  {
    mock.toggleTimer.setTimedOut(true);
    logic.handleToggleTimerTimeout();
  ...
  // Case 7 - Simulate temperature timer timeout, expect the LED to be
  // unaffected.
  {
...
TEST(Logic, TempHandling) {
  ...
  // Case 1 - Press the toggle button, simulate button event.
  {
    ...
    // Expect the temperature to not be printed, since the wrong button was
    // pressed.
 ...
 */
TEST(Logic, Eeprom) {
  // Case 1 - Verify that the toggle timer is disabled at startup if its EEPROM
  // bit is not set. This simulates the timer being disabled before the last
  // poweroff.
  {
    // Create logic implementation and run the system.
    Mock mock{};
  ...
  // Case 2 - Verify that the toggle timer is enabled at startup if its EEPROM
  // bit is set. This simulates the timer being enabled before the last
  // poweroff.
  {
